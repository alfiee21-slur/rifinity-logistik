...
export default function InboundHistoryPage() {
  ...
  const fetchHistory = () => {
    ...
    fetch(`/api/inbound-history?${params.toString()}`)
      .then(res => res.json())
      .then(data => {
  ...
  const uniqueSuppliers = Array.from(new Set(transactions.map(tx => tx.supplier || tx.item?.supplier || 'Unknown'))).filter(s => s !== 'Unknown')
  const totalSuppliersCount = uniqueSuppliers.length

  // Top rack storage zone
  const rackCounts = transactions.reduce((acc: any, curr: any) => {
    const loc = curr.item?.rackLocation || 'TBD'
    acc[loc] = (acc[loc] || 0) + 1
    return acc
  }, {})
  ...
  Object.keys(rackCounts).forEach(key => {
    if (rackCounts[key] > maxRackCount) {
      maxRackCount = rackCounts[key]
      topRack = key
    }
  })
    ...
    <div className="outbound-history-container" style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="print-report-header" style={{ display: 'none' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '16px', fontFamily: 'sans-serif' }}>
          <h2 style={{ fontSize: '12px', fontWeight: 800, margin: '0 0 4px 0', textTransform: 'uppercase', textDecoration: 'underline', color: '#000000' }}>
            {t('official_inbound_report_title')}
          </h2>
        ...
        <div style={{ display: 'flex', gap: '5%', marginBottom: '16px' }}>
          ...
          <div style={{ width: '48%' }}>
            <h3 style={{ fontSize: '9px', fontWeight: 900, borderBottom: '1px solid #000000', paddingBottom: '3px', margin: '0 0 6px 0', textTransform: 'uppercase' }}>{t('print_section_1')}</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '8px' }}>
              <tbody>
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0' }}>{t('total_inbound_goods')} (Qty)</td>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0', textAlign: 'right', fontWeight: 'bold' }}>{totalPackages} unit</td>
                </tr>
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '2px 0 2px 10px', color: '#4b5563', fontSize: '7.5px' }}>↳ {t('internal_pt')}</td>
                ...
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '2px 0 2px 10px', color: '#d97706', fontSize: '7.5px' }}>↳ {t('client_consignment')}</td>
                ...
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0' }}>{t('total_purchase_valuation')}</td>
                ...
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '2px 0 2px 10px', color: '#4b5563', fontSize: '7.5px' }}>↳ {t('pt_expense_capital')}</td>
                ...
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '2px 0 2px 10px', color: '#d97706', fontSize: '7.5px' }}>↳ {t('client_consigned_assets')}</td>
                ...
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0' }}>{t('total_suppliers')}</td>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0', textAlign: 'right', fontWeight: 'bold' }}>{totalSuppliersCount} {t('suppliers_count')}</td>
                </tr>
                <tr>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0' }}>{t('main_warehouse')}</td>
                  <td style={{ borderBottom: '1px solid #dddddd', padding: '3px 0', textAlign: 'right', fontWeight: 'bold', textTransform: 'uppercase' }}>{topRack}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div style={{ width: '48%' }}>
            <h3 style={{ fontSize: '9px', fontWeight: 900, borderBottom: '1px solid #000000', paddingBottom: '3px', margin: '0 0 6px 0', textTransform: 'uppercase' }}>{t('print_section_2')}</h3>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '8px' }}>
              <tbody>
        ...
        <h3 className={!includeDetails ? 'print-hidden' : ''} style={{ fontSize: '9px', fontWeight: 900, borderBottom: '1px solid #000000', paddingBottom: '3px', margin: '0 0 6px 0', textTransform: 'uppercase' }}>{t('print_section_3')}</h3>
      </div>
      ...
      <header className="print-hidden screen-header-layout" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <PackagePlus size={24} style={{ color: 'var(--accent)' }} />
            {t('inbound_history_title')}
          </h1>
          <p className="page-subtitle">{t('inbound_history_desc')}</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'rgba(255,255,255,0.7)', fontSize: '0.8rem', cursor: 'pointer', userSelect: 'none' }}>
            ...
            />
            {t('include_pdf_details')}
          </label>
          <button 
            onClick={() => window.print()}
            className="btn-primary"
            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'var(--accent)' }}
          >
            <Printer size={16} />
            {t('export_pdf_btn')}
          </button>
      ...
      <div className="print-hidden stats-grid-layout" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
        ...
        <div className="card glass" style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>{t('total_inbound_goods')}</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '0.25rem' }}>
            <h3 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--accent)' }}>{totalPackages.toLocaleString('id-ID')} <span style={{ fontSize: '0.875rem', fontWeight: 500, color: 'rgba(255,255,255,0.7)' }}>pcs</span></h3>
            <span className="badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.7rem', color: 'var(--accent)', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
              <ArrowUpRight size={10} style={{ transform: 'rotate(180deg)' }} /> IN
            </span>
          </div>
        ...
        <div className="card glass" style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '0.25rem' }}>
            <h3 style={{ fontSize: '1.75rem', fontWeight: 800, color: 'var(--accent)' }}>{totalAvailable.toLocaleString('id-ID')} <span style={{ fontSize: '0.875rem', fontWeight: 500, color: 'rgba(255,255,255,0.5)' }}>pcs</span></h3>
            <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>On-Hand Stock</span>
          </div>
        </div>

        <div className="card glass" style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>{t('total_purchase_valuation')}</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '0.25rem' }}>
            <h3 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#34d399' }}>Rp {totalValue.toLocaleString('id-ID')}</h3>
            <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>Total Barang</span>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem', marginTop: '0.4rem', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '0.4rem', fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>{t('pt_expense_capital')}:</span>
              <strong style={{ color: '#f87171' }}>Rp {totalInternalValue.toLocaleString('id-ID')}</strong>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span>{t('client_consigned_assets')}:</span>
        ...
        <div className="card glass" style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>{t('total_suppliers')}</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '0.25rem' }}>
            <h3 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#60a5fa' }}>{totalSuppliersCount} <span style={{ fontSize: '0.875rem', fontWeight: 500, color: 'rgba(255,255,255,0.5)' }}>{t('suppliers_count')}</span></h3>
            <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>{t('active_partners')}</span>
          </div>
        </div>

        <div className="card glass" style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
          <span style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', fontWeight: 500 }}>{t('most_active_rack_zone')}</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '0.25rem' }}>
            <h3 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fbbf24', textTransform: 'uppercase' }}>{topRack}</h3>
            <span style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)' }}>{t('main_warehouse')}</span>
          </div>
        </div>
      ...
      <div className="card glass print-hidden" style={{ padding: '1.25rem' }}>
        <form className="filter-form-layout" onSubmit={handleSearchSubmit} style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', width: '100%' }}>
          ...
          <div className="filter-search-input-wrapper" style={{ position: 'relative', flex: 1, minWidth: '240px' }}>
            ...
            <input 
              type="text" 
              placeholder={t('inbound_search_placeholder')} 
              value={search}
          ...
          <div className="filter-buttons-wrapper" style={{ display: 'flex', gap: '0.75rem', marginLeft: 'auto' }}>
            <button 
              type="submit" 
              className="btn-primary" 
              style={{ padding: '0.6rem 1.25rem', fontSize: '0.85rem', background: 'var(--accent)' }}
            >
              {t('search')}
            </button>
            <button 
              ...
              style={{ padding: '0.6rem 1.25rem', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}
            >
              <RefreshCw size={14} />
              {t('reset')}
            </button>
      ...
      <div className="card glass printable-report-card responsive-table-card" style={{ padding: '1.25rem', overflowX: 'auto' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', gap: '1rem', color: 'rgba(255,255,255,0.4)' }}>
            ...
            <span style={{ fontSize: '0.875rem' }}>{t('loading_inbound_history')}</span>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', gap: '0.75rem', color: 'rgba(255,255,255,0.3)', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '0.8rem', maxWidth: '300px' }}>{t('inbound_history_empty_desc')}</p>
          </div>
        ) : (
          <table className={`printable-report-table ${!includeDetails ? 'print-hidden' : ''}`} style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem', color: 'white' }}>
            <thead>
              <tr style={{ borderBottom: '1.5px solid rgba(255,255,255,0.08)' }}>
                ...
                <th style={{ textAlign: 'right', padding: '0.75rem 0.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>{t('table_purchase_value')}</th>
                <th style={{ textAlign: 'left', padding: '0.75rem 0.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>{t('table_supplier')}</th>
                <th style={{ textAlign: 'left', padding: '0.75rem 0.5rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>{t('table_storage_rack')}</th>
                ...
                >
                  ...
                  <td data-label={t('table_quantity')} style={{ padding: '0.85rem 0.5rem', verticalAlign: 'top', textAlign: 'center' }}>
                    <div className="td-wrapper">
                      <span className="badge" style={{ display: 'inline-flex', padding: '2px 6px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600, color: 'var(--accent)', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                  ...
                  {/* Goods Value */}
                  <td data-label={t('table_purchase_value')} style={{ padding: '0.85rem 0.5rem', verticalAlign: 'top', textAlign: 'right', fontWeight: 600, color: '#34d399', whiteSpace: 'nowrap' }}>
                    <div className="td-wrapper" style={{ fontWeight: 600, color: '#34d399' }}>
                      Rp {tx.totalValue.toLocaleString('id-ID')}
                    </div>
                  </td>

                  {/* Supplier Details */}
                  <td data-label={t('table_supplier')} style={{ padding: '0.85rem 0.5rem', verticalAlign: 'top', fontSize: '0.8rem', color: 'rgba(255,255,255,0.8)' }}>
                    <div className="td-wrapper" style={{ display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontWeight: 600, color: 'white' }}>
                        ...
                        <span>{tx.supplier || tx.item?.supplier || t('general_vendor')}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', color: 'rgba(255,255,255,0.4)', marginTop: '4px', fontSize: '0.72rem' }}>
                        <Truck size={10} style={{ flexShrink: 0 }} />
                        <span>{t('official_distribution')}</span>
                      </div>
                    </div>
                  </td>

                  {/* Storage Details */}
                  <td data-label={t('table_storage_rack')} style={{ padding: '0.85rem 0.5rem', verticalAlign: 'top', fontSize: '0.8rem' }}>
                    <div className="td-wrapper" style={{ display: 'flex', flexDirection: 'column' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', fontWeight: 600, color: 'white' }}>
                        <Boxes size={12} style={{ color: '#fbbf24', flexShrink: 0 }} />
                        <span style={{ textTransform: 'uppercase' }}>{tx.item?.rackLocation || 'ZONA-A / RAK-01'}</span>
                      </div>
